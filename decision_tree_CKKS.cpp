		...
		InputReader(string filename) {
			...
			if (file.is_open()) {
				...
				while (getline(file, line)) {
					...
					data_Double.push_back(numericRow);
				}
			...
			for (int i = 0; i < static_cast<int>(data_Double.size()); i++) {
				vector<double> row = data_Double[i];
				vector<double> row_not_enc = data_Double[i];
				vector<Ciphertext<DCRTPoly>> row_enc;
				Ciphertext<DCRTPoly> row_enc_i;
				for (auto& row_i : row) {
					row_enc_i = encCKKS(row_i);
...
int main() {
	...
	/*
	   InputReader answerInputReader("Answ_acc.txt");
